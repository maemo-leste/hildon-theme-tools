...
/* Save image as jpeg or png with optional dithering. */
void                            save_pixbuf (GdkPixbuf *pixbuf, gchar *filename, gboolean dither)
{
        if (dither) {
                dither_pixbuf (pixbuf);
        }

        if ((strlen (filename) >= 4 && strcmp (filename + (strlen (filename) - 4), ".jpg") == 0) ||
             (strlen (filename) >= 5 && strcmp (filename + (strlen (filename) - 5), ".jpeg") == 0))
